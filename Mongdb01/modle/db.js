/**
 * 这里写所有对数据库的操作
 */
var MongoClient = require('mongodb').MongoClient;
var assert = require('assert');
var setting = require('../setting.js')

// 链接数据库
function __connectDB(callback) {
    var DB_url = setting.dburl; // 数据库地址

    MongoClient.connect(DB_url, function (err, db) {
        callback(err, db);
    })
}

/**
 * 插入一条数据
 * @param {*} collectionName 集合名
 * @param {*} Datajson 写入的json数据
 * @param {*} callback 回调函数
 */
function __insertOne(collectionName, Datajson, callback) {
    __connectDB(function (err, db) {
        db.collection(collectionName).insertOne(Datajson, function (err, result) {
            callback(err, result); // 通过回调函数上传数据
            db.close();
        })
    })
}

/**
 * 查找全部数据
 * @param {*} collectionName 集合名
 * @param {*} JsonObj 查询的对象
 * @param {*} callback 回调函数
 */

function __find(collectionName, JsonObj, callback) {
    var result = [];
    if (arguments.length != 3) {
        callback('find函数里参数必须为三个哦', null);
        return;
    }
    __connectDB(function (err, db) {
        var cursor = db.collection(collectionName).find(JsonObj);
        cursor.each(function (err, doc) {
            assert.equal(err, null);
            if (doc != null) {
                result.push(doc);
            } else {
                callback(null, result);
            }
        })
    });
}

/**
 * 分页查找数据
 * @param {*} collectionName 集合名
 * @param {*} JsonObj 查询的对象
 * @param {*} PageAgrvs 具体查询对象
 * @param {*} callback 回调函数
 * 
 */

function __findByPage(collectionName, JsonObj, C, D) {
    var result = []; //结果数组
    if (arguments.length == 3) {
        //那么参数C就是callback，参数D没有传。
        var callback = C;
        var skipnumber = 0;
        //数目限制
        var limit = 0;
    } else if (arguments.length == 4) {
        var callback = D;
        var args = C;
        //应该省略的条数
        var skipnumber = args.pageamount * args.page || 0;
        //数目限制
        var limit = args.pageamount || 0;
        //排序方式
        var sort = args.sort || {};
    } else {
        throw new Error("find函数的参数个数，必须是3个，或者4个。");
        return;
    }

    //连接数据库，连接之后查找所有
    __connectDB(function (err, db) {
        var cursor = db.collection(collectionName).find(JsonObj).skip(skipnumber).limit(limit).sort(sort);
        cursor.each(function (err, doc) {
            if (err) {
                callback(err, null);
                db.close(); //关闭数据库
                return;
            }
            if (doc != null) {
                result.push(doc); //放入结果数组
            } else {
                //遍历结束，没有更多的文档了
                callback(null, result);
                db.close(); //关闭数据库
            }
        });
    });
}


/**
 * 删除
 * @param {*} collectionName 集合名
 * @param {*} json 查询的对象
 * @param {*} callback 回调函数
 * 
 */

function __DeleteMany(collectionName, json, callback) {
    __connectDB(function (err, db) {
        //删除
        db.collection(collectionName).deleteMany(
            json,
            function (err, results) {
                callback(err, results);
                db.close(); //关闭数据库
            }
        );
    });
}


/**
 * 修改
 * @param {*} collectionName 集合名
 * @param {*} json1 查询的对象
 * @param {*} json2 查询的对象
 * @param {*} callback 回调函数
 * 
 */

function __UpdateMany(collectionName, json1, json2, callback) {
    __connectDB(function (err, db) {
        //删除
        db.collection(collectionName).updateMany(
            json1,
            json2,
            function (err, results) {
                callback(err, results);
                db.close();
            });
    });
}

/**
 * 获取总数
 * @param {*} collectionName 集合名
 * @param {*} callback 回调函数
 * 
 */

function __getAllCount(collectionName, callback) {
    __connectDB(function (err, db) {
        //删除
        db.collection(collectionName).count({}).then(function(count) {
            callback(count);
            db.close();
        });
    });
}

/****
 * 暴露出去
 * 
 * 
 */

module.exports = {
    __insertOne,
    __find,
    __findByPage,
    __DeleteMany,
    __UpdateMany,
    __getAllCount
}